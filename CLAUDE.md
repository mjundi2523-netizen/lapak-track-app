...
## Tema & dark mode — ditambah 2026-06-30
- **Warna primer = cyan `#0891b2`** (CSS var `--lt-p`, ganti dari indigo `#4f39f6`). Hampir semua aksen pakai `var(--lt-p)`; tint cyan untuk pill = `#cffafe`/teks `#0e7490`, gradient hero = `var(--lt-p)→#22d3ee`. Class Tailwind: `cyan-50/300/400`.
- **Dark mode (disimpan di DB)**: preferensi per-user di tabel **`config_users`** (`user_id` unik, `dark_mode` bool) — model `ConfigUser`, relasi `User::config()` (hasOne) + helper `User::prefersDark()`. Class **`lt-dark` ditaruh di `<body>`** dan **dirender server** dari DB (`<body @class(['lt-dark'=>auth()->user()?->prefersDark()])>` di `layouts/app.blade.php`). Di `<body>` (bukan `<html>`) supaya **ikut ter-morph & persist saat `wire:navigate`** — sebelumnya pakai `<html>`+localStorage dan ke-reset tiap pindah halaman. Toggle di **halaman Profil** (kartu "Tampilan"): Alpine optimistik `document.body.classList.toggle('lt-dark')` (feedback instan) + `$wire.setDark($on)` → `ShowProfile::setDark()` `updateOrCreate` ke `config_users`. Tak ada flash karena server sudah render kelasnya. Sidebar memang sudah gelap → dibiarkan. Login (guest-layout) selalu terang.
- **CSS dark** = blok override di akhir `resources/css/app.css` scoped `body.lt-dark`, menyasar nilai nyata app (hex inline via `[style*="..."]` + class arbitrary `text-[#..]`/`bg-white` yang di-escape). Hanya aktif saat `lt-dark` → mode terang tak terpengaruh. Status-pill warna (hijau/merah/biru) sengaja dibiarkan (tetap terbaca).
- **Komponen MaryUI/DaisyUI** (`x-card`/`x-input`/`x-select`/`x-button` di form) TIDAK kena CSS custom di atas → digerakkan oleh **`data-theme="dark"` di `<body>`** (juga dirender server dari DB + di-set Alpine saat toggle). Tema `dark` sudah terdaftar di `@plugin "daisyui"`. Input mentah non-MaryUI (form Profil) ditangani via class `.bg-white`/`.text-[#..]` (lihat `input.bg-white` di app.css), bukan dipaksa global, agar tak dua-warna dengan input DaisyUI.
- **Warna primer DaisyUI** (`--color-primary`, dipakai `btn-primary` MaryUI) di-override ke cyan `#0891b2` di `app.css` (`[data-theme=light/dark]`) supaya tombol MaryUI ikut brand cyan (sebelumnya biru/ungu default DaisyUI), light & dark.
- **Profil** (`Profile/ShowProfile` + `livewire/profile/show-profile.blade.php`) di-rebuild ikut Claude Design: header gradient + avatar inisial + statistik (Lapak/Pedagang real), form Informasi Profil & Ubah Password (selalu tampil), kartu Tampilan (dark toggle), Detail Akun (status/verifikasi/paket/bergabung/ID), Tentang Aplikasi, tombol Keluar. Fitur lama dipertahankan; "Zona Berbahaya/hapus akun" desain TIDAK diadopsi (fitur destruktif belum ada).
...
## Pengeluaran Rutin (auto-generate) — ditambah 2026-07-01
- **`recurring_expenses`** (rxid, ecid, title, amount, frequency, interval_count, payment_method, start_date, `auto_post`, `is_active`, `generated_until`, note, created_by/modified_by) = **template** pengeluaran berulang. Model `RecurringExpense`. Menu **Master Data → "Pengeluaran Rutin"** (`recurring-expenses.*`, **premium-gated**; template = master data, sejajar `payment_terms`). CRUD: Index/Create/Edit + toggle aktif (tanpa delete; nonaktifkan saja).
- **`expenses`** ditambah **`rxid`** (nullable FK→`recurring_expenses`, `nullOnDelete`; null = manual) + **`status`** enum `posted|pending` (default `posted`). Occurrence hasil generate menempel ke templatenya.
- **Mesin** `App\Services\ExpenseGenerationService` (mirror `BillGenerationService`, lazy roll-forward). **Cursor = `recurring_expenses.generated_until`** (tanggal occurrence terakhir), **sengaja DEKOPEL dari baris `expenses`** supaya Tunda (ubah `expense_date`) / Batalkan (hapus occurrence) **tidak** memicu regenerasi. Catch-up dipanggil di `mount()` Dashboard, IndexExpenses, dan semua layar RecurringExpenses. Belum ada cron; ada `advance()` per frekuensi×interval.
- **Mode per-template `auto_post`**: `true` → occurrence dibuat `posted` langsung (biaya tetap: gaji/sewa). `false` → dibuat `pending` (draft, biaya variabel: listrik/air) menunggu konfirmasi nominal aktual.
- **`pending` DIKECUALIKAN dari semua hitungan** (belum jadi pengeluaran sah): total IndexExpenses, CashFlow (`$expense`/`$byCategory`), Dashboard (`heroExpense`/chart `$spent`) semua difilter `status='posted'`. Di IndexExpenses baris pending tampil badge "Menunggu Konfirmasi" + "Rutin"; aksinya link ke halaman Pengeluaran Rutin (bukan void).
- **Popup konfirmasi** (di `IndexRecurringExpenses`, auto-muncul saat `mount` bila ada `pending` dengan `expense_date <= hari ini`): modal tengah, per-occurrence 3 aksi — **Konfirmasi** (`confirmPending`: set nominal aktual dari `confirmAmounts[xpid]` → `posted`), **Tunda** (`snoozePending`: set `expense_date` ke `snoozeDates[xpid]`, wajib > hari ini, tetap `pending` → hilang dari popup s/d tanggal itu), **Batalkan** (`cancelPending`: hard-delete occurrence; aman karena cursor `generated_until` tak mundur). Popup nutup sendiri saat tak ada lagi yang due.

## Navigasi form "kembali ke asal" — ditambah 2026-07-04
- **Trait `App\Livewire\Concerns\ReturnsBack`** dipakai SEMUA form create/edit/void (16 komponen: Payments, Dealers, Stalls, PaymentTerms, AddOns, Expenses, ExpenseCategories, RecurringExpenses). `mountReturnsBack()` merekam **referer** (hanya same-origin, bukan URL form sendiri) ke `$backUrl`; setelah simpan panggil **`redirectBack('fallback.route')`** (bukan `redirect(route(...))`), tombol **Batal** di view pakai **`$this->backHref('fallback.route')`**. Hasil: selesai isi form → balik persis ke halaman asal, seperti tombol back (mis. bayar dari Tagihan → balik ke Tagihan; dari Pembayaran → balik ke Pembayaran).
