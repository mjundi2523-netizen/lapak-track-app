<div>
    <x-page-heading title="Catat Pemasukan" />

    <x-card class="max-w-[680px]">
        <x-form wire:submit="save">
            <x-input label="Judul" wire:model="title" placeholder="Mis. Sewa panggung acara" required />

            <x-select label="Kategori" wire:model="icid"
                :options="$categories->map(fn($c) => ['id' => $c->icid, 'name' => $c->name])"
                option-value="id" option-label="name"
                placeholder="{{ $categories->isEmpty() ? 'Belum ada kategori — buat dulu di Kategori Pemasukan' : 'Pilih kategori' }}" required />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-input label="Tanggal" wire:model="income_date" type="date" :max="now()->format('Y-m-d')" required />
            ...
            <x-slot:actions>
                <x-button label="Batal" link="{{ $this->backHref('incomes.index') }}" class="btn-ghost" />
