...
<div>
    <x-index-header title="Pemasukan Tambahan">
        ...
        <x-select wire:model.live="categoryFilter" placeholder="Semua Kategori"
            :options="$categories->map(fn($c) => ['id' => $c->icid, 'name' => $c->name])" option-value="id" option-label="name" class="w-44" />
        ...
        <x-button label="Tambah" link="{{ route('incomes.create') }}" class="btn-primary" icon="o-plus" />
    </x-index-header>

    {{-- Ringkasan total (sesuai filter, hanya yang aktif) --}}
    <div class="mb-4 inline-flex items-center gap-3 rounded-[12px] px-5 py-3"
         style="background:linear-gradient(135deg,#059669,#10b981); box-shadow:0 8px 20px rgba(5,150,105,0.20);">
        <x-icon name="o-arrow-trending-up" class="w-6 h-6 text-white/90" />
        <div>
            <div class="text-[11px] font-medium text-white/85 uppercase tracking-wide">Total Pemasukan (filter)</div>
    ...
    <div class="lt-card-table">
        <table class="lt-table">
            <thead>
                <tr>
                    @include('partials.sort-th', ['field' => 'income_date', 'label' => 'Tanggal'])
            ...
            <tbody>
                @forelse($incomes as $row)
                    @php $c = $catColors[($row->icid ?? 0) % count($catColors)]; @endphp
                    <tr class="lt-row {{ $row->is_voided ? 'lt-row-danger' : '' }}">
                        <td class="lt-td">{{ $row->income_date?->format('d-m-Y') ?? '-' }}</td>
                        ...
                        <td class="lt-td">
                            <div class="flex gap-1 justify-end">
                                @if(! $row->is_voided)
                                    <a href="{{ route('incomes.void', $row) }}" wire:navigate class="lt-act text-error" title="Batalkan"><x-icon name="o-x-mark" class="w-[18px] h-[18px]" /></a>
                                @endif
                            </div>
                        </td>
                    </tr>
        ...
        <div class="lt-table-foot">{{ $incomes->links() }}</div>
    </div>
</div>
